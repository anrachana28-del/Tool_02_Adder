...
app.use(cors()); // enable CORS for cloud
...
async function autoCheck(){
  await Promise.all(accounts.map(acc => checkTGAccount(acc)));
}
setInterval(autoCheck,60000);
autoCheck();

// ===== Get Available Account (Smart Rotation) =====
function getAvailableAccount(){
  const now = Date.now();
  // sort active accounts by earliest ready
  const active = accounts.filter(a=>a.status==="active" && (!a.floodWaitUntil || a.floodWaitUntil<now));
  if(!active.length) return null;
  return active.sort((a,b)=> (a.floodWaitUntil||0) - (b.floodWaitUntil||0))[0];
}
...
app.post('/members', async(req,res)=>{
  try{
    ...
    if(!acc) return res.status(503).json({error:"No active account"});
...
app.post('/export-members', async(req,res)=>{
  try{
    ...
    if(!targetGroup) return res.status(400).json({error:"Missing targetGroup"});
    const acc = getAvailableAccount();
    if(!acc) return res.status(503).json({error:"No active account"});
    const client = await getClient(acc);
...
// ===== Add Member Endpoint (Smart Rotation) =====
app.post('/add-member', async(req,res)=>{
  try{
    ...
    if(targetMembers.includes(id)){
      return res.json({status:"skipped", reason:"already in target group", accountUsed:"none", silent:true});
    }
    ...
    try{
      ...
      // random delay after success (avoid flood)
